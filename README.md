# Find a Housing Counselor data pipeline
...
This repo contains the code we use to generate the JSON and PDF files the Find a Housing Counselor tool relies on.

At a high level, this consists of two management commands:
...
We pass these html files to a pdf generator so that pdfs are easily downloadable at stable endpoints.
...
A daily job queries HUD data and produces the JSON and PDF files we use for the Find a Housing Counselor tool.
...
The `GEOCODE_ZIPCODES` step generates a file of all ZIP codes in the United States and their location latitude and longitude and saves it.
By default, this step is not enabled.
Since ZIP code geographical information rarely changes, we run this step rarely.

If enabled, this step calls the [`hud_geocode_zipcodes`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/management/commands/hud_geocode_zipcodes.py) management command.
The management command in turn calls the `BulkZipCodeGeocoder` in [`geocoder.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/geocoder.py).
...
The management command inserts this data into a CSV and saves it to `./zipcodes.csv`.
...
This step calls the [`hud_generate_json`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/management/commands/hud_generate_json.py) management command,
...
(_in [`fetcher.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/fetcher.py)_)
...
(_in [`results_archiver.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/results_archiver.py)_)

Save a copy of the full set of results as a zip file in the home directory.
Then we transfer the zip file to S3 as part of the `MAKE JSON` step.
...
(_in [`cleaner.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/cleaner.py)_)
...
(_in [`geocoder.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/geocoder.py)_)
...
This uses the data file created in the Geocode ZIP Codes stage ([above](#geocode-zip-codes)).

#### Create collections of results by ZIP code

(_in [`generator.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/generator.py)_)
...
Save the resulting JSON files in a `jsons` directory, e.g. `jsons/12345.json`.
...
It saves them in an `htmls` directory.
This step is enabled by default.

This step calls the [`hud_generate_html`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/management/commands/hud_generate_html.py) management command,
which calls HTML generation code in [`generator.py`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/generator.py).
Django renders the [`housing_counselor/pdf_selfcontained.html`](https://github.com/cfpb/housing-counselor-pipeline/blob/main/housing_counselor/pdf_selfcontained.html) template with the housing counselor data from each JSON file.
We save the resulting HTML files in a `htmls` directory, e.g. `htmls/12345.html`.

### Generate PDFs

When enabled, the `MAKE_PDF` step generates a PDF of each file created in the previous step.
It saves them in a `pdfs` directory.
This step is enabled by default.
